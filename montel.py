...
from MontelLexer import *
from MontelParser import *
...
class Number(Value):
    ...
    def divided_by(self, other):
        if isinstance(other, Number):
            if other.value == 0:
                error = RTError(other.pos_start, other.pos_end,
                'Division by zero!',
                self.context
                )
                return None, error
...
BuiltInFunction.clear       = BuiltInFunction("cls")
...
global_symbol_table = SymbolTable() # Нужна чтобы сохранять переменные до завершения работы программы
...
def run(fn, text):
    ...
    #Создаём AST (Абстрактное семантическое дерево)
    ...
    interpreter = Interpreter()
    context = Context('<program>') #Название контекста
    context.symbol_table = global_symbol_table #При изменение таблицы внутри контекста, изменяется и глобальная таблица
